...
public class ProtossDragoonUpgradeOrder extends UpgradeOrder {
	@Override
	public void execute() {
		...
		super.order(UnitType.Protoss_Robotics_Support_Bay, UpgradeType.Gravitic_Drive, new OrderCondition() {
			...
			public boolean isActive() {
				// TODO Auto-generated method stub
				if (BattleUnitGroupManager.instance().getBattleUnitGroup(UnitType.Protoss_Reaver).getUnitCount() >= 2 &&
						MyBotModule.Broodwar.self().minerals() >= 200 && MyBotModule.Broodwar.self().gas() >= 200) {
					return true;
				}
				return false;
